...
// CalcInheritStrategies 根据模板的继承关系，找到每个机器对应的模板全量
func CalcInheritStrategies(allTpls map[int]*model.Template, tids []int, tpl2Strategies map[int][]*model.Strategy) []model.Strategy {
	...
	for _, tid := range tids {
		ids := cache.ParentStrategyIds(allTpls, tid)
		if len(ids) <= 0 {
			continue
		}
	...
	for i := 0; i < count; i++ {
		...
		for j := 0; j < count; j++ {
			...
			if slice.SliceIntEq(tpl_buckets[i], tpl_buckets[j]) {
				break
			}
			if slice.SliceIntLt(tpl_buckets[i], tpl_buckets[j]) {
				valid = false
				break
			}
		}
	...
	for _, bucket := range uniq_tpl_buckets {
		...
		for _, tid := range bucket {
			...
			if stras, ok := tpl2Strategies[tid]; ok {
				for _, s := range stras {
					uuid := fmt.Sprintf("metric:%s/tags:%v", s.Metric, tag.SortedTags(s.Tags))
...
func (t *Hbs) GetStrategies(req model.NullRpcRequest, reply *model.StrategiesResponse) error {
	...
	// 一个机器ID对应多个模板ID
	hid2Tids := cache.HostTemplateIds.GetMap()
	sz := len(hid2Tids)
	if sz == 0 {
		return nil
	}
	...
	for hostId, tplIds := range hid2Tids {
