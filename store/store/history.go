...
func GetLastItem(key string) *model.StoreItem {
	...
	return first.(*model.StoreItem)
}

func GetAllItems(key string) []*model.StoreItem {
	ret := make([]*model.StoreItem, 0)
	itemlist, found := HistoryCache.Get(key)
	if !found || itemlist == nil {
		return ret
	}
	...
	for _, item := range all {
		...
		ret = append(ret, item.(*model.StoreItem))
	}
	return ret
}

func AddItem(key string, val *model.StoreItem) {
	itemlist, found := HistoryCache.Get(key)
	...
	first := slist.Front()
	if first == nil || first.(*model.StoreItem).Timestamp < val.Timestamp { // first item or latest one
