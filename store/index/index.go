...
func ReceiveItem(item *model.StoreItem, md5 string) {
	if item == nil {
		return
	}
	...
	rrdFileName := utils.RrdFileName(cc.Config().RRD.Storage, md5, item.DsType, item.Step)
	if utils.IsRrdFileExist(rrdFileName) {
		IndexedItemCache.Put(md5, NewIndexCacheItem(uuid, item))
		return
	}
...
func RemoveItem(item *model.StoreItem) {
	md5 := item.Checksum()
	...
	key := utils.FormRrdCacheKey(checksum, item.DsType, item.Step)
	poped_items := store.StoreItems.PopAll(key)
