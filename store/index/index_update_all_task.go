...
func updateIndexAll(updateStepInSec int64) int {
	...
	for _, key := range keys {
		...
		go func(gitem *model.StoreItem, dbConn *sql.DB) {
...
// 根据item,更新mysql
func updateIndexFromOneItem(item *model.StoreItem, conn *sql.DB) error {
	if item == nil {
		return nil
	}
	...
	counter := item.Metric
	if len(item.Tags) > 0 {
		counter = fmt.Sprintf("%s/%s", counter, tag.SortedTags(item.Tags))
	}
