...
// 利用rrdtool设置归档策略
func create(filename string, item *model.StoreItem) error {
	now := time.Now()
...
func update(filename string, items []*model.StoreItem) error {
...
func flushrrd(filename string, items []*model.StoreItem) error {
	if items == nil || len(items) == 0 {
		return errors.New("empty items")
	}

	if !utils.IsRrdFileExist(filename) {
...
func FlushFile(filename, md5 string, items []*model.StoreItem) error {
	done := make(chan error, 1)
...
func Fetch(filename string, md5 string, cf string, start, end int64, step int) ([]*model.RRDData, error) {
	done := make(chan error, 1)
	task := &io_task_t{
...
func FlushAll(force bool) {
	n := store.StoreItems.Size / 10
	for i := 0; i < store.StoreItems.Size; i++ {
		FlushRRD(i, force)
		if i%n == 0 {
			log.Printf("flush hash idx:%03d size:%03d disk:%08d net:%08d\n",
				i, store.StoreItems.Size, disk_counter, net_counter)
		}
	}
...
func CommitByKey(key string) {
	md5, dsType, step, err := utils.SplitRrdCacheKey(key)
	if err != nil {
		return
	}
	filename := utils.RrdFileName(cc.Config().RRD.Storage, md5, dsType, step)

	items := store.StoreItems.PopAll(key)
	if len(items) == 0 {
		return
	}
...
func PullByKey(key string) {
	...
	item := store.StoreItems.First(key)
	if item == nil {
		return
	}
...
func FlushRRD(idx int, force bool) {
	...
	keys := store.StoreItems.KeysByIndex(idx)
	if len(keys) == 0 {
		return
	}

	for _, key := range keys {
		flag, _ := store.StoreItems.GetFlag(key)

		//write err data to local filename
		if force == false && cc.Config().Migrate.Enabled && flag&gg.GRAPH_F_MISS != 0 {
			if time.Since(begin) > time.Millisecond*gg.FLUSH_DISK_STEP {
...
func shouldFlush(key string) bool {
	...
	if store.StoreItems.ItemCnt(key) >= gg.FLUSH_MIN_COUNT {
		return true
	}

	deadline := time.Now().Unix() - int64(gg.FLUSH_MAX_WAIT)
	back := store.StoreItems.Back(key)
