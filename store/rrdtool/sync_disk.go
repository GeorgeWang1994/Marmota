...
func InitChannel() {
	...
	ioWorkerNum := cc.Config().IOWorkerNum
...
func syncDisk() {
	time.Sleep(time.Second * gg.CACHE_DELAY)
	ticker := time.NewTicker(time.Millisecond * gg.FLUSH_DISK_STEP)
	defer ticker.Stop()
	var idx int = 0
			...
			for {
				...
				case task := <-io_task_chans[i]:
					if task.method == IO_TASK_M_READ {
						...
						if args, ok := task.args.(*gg.File); ok {
