...
func migrate_start(cfg *cc.GlobalConfig) {
	var err error
	var i int
	if cfg.Migrate.Enabled {
		Consistent.NumberOfReplicas = cfg.Migrate.Replicas

		nodes := mmap.KeysOfMap(cfg.Migrate.Cluster)
				...
				if atomic.LoadInt32(&flushrrd_timeout) != 0 {
					...
					if err = fetch_rrd(client, task.Key, addr); err != nil {
						if os.IsNotExist(err) {
							...
							store.StoreItems.SetFlag(task.Key, 0)
...
func query_data(client **rpc.Client, addr string,
	...
	for i = 0; i < 3; i++ {
		err = rpc_call(*client, "Graph.Query", args, resp,
			time.Duration(cc.Config().CallTimeout)*time.Millisecond)
...
func send_data(client **rpc.Client, key string, addr string) error {
	...
	//remote
	if flag, err = store.StoreItems.GetFlag(key); err != nil {
		return err
	}
	cfg := cc.Config()

	store.StoreItems.SetFlag(key, flag|gg.GRAPH_F_SENDING)
	...
	// err
	store.StoreItems.PushAll(key, items)
	//flag |= g.GRAPH_F_ERR
out:
	flag &= ^gg.GRAPH_F_SENDING
	store.StoreItems.SetFlag(key, flag)
	return err
...
func fetch_rrd(client **rpc.Client, key string, addr string) error {
	...
	store.StoreItems.SetFlag(key, flag|gg.GRAPH_F_FETCHING)

	md5, dsType, step, _ = utils.SplitRrdCacheKey(key)
	filename = utils.RrdFileName(cfg.RRD.Storage, md5, dsType, step)
