...
func PluginRun(plugin *Plugin) {
	...
	fpath := filepath.Join(cc.Config().Plugin.Dir, plugin.FilePath)
	args := plugin.Args
	...
	if errStr != "" {
		logFile := filepath.Join(cc.Config().Plugin.LogDir, plugin.FilePath+"("+plugin.Args+")"+".stderr.log")
	...
	transfer.SendToTransfer(metrics)
}
