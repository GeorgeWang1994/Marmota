...
func SyncMinePlugins() {
	...
	if !cc.Config().Heartbeat.Enabled {
		return
	}

	if cc.Config().Heartbeat.Addr == "" {
		return
	}
...
func syncMinePlugins() {
	...
	for {
		...
		err = gg.HBSRpcClient().Call("Agent.MinePlugins", req, &resp)
		if err != nil {
		...
		for _, script_file := range filefmt_scripts {
			abs_path := filepath.Join(cc.Config().Plugin.Dir, script_file[0])
